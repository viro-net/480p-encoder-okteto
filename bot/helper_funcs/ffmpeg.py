...
async def convert_video(video_file, output_directory, total_time, bot, message, chan_msg):
    ...
    file_genertor_command = f'ffmpeg -hide_banner -loglevel quiet -progress "{progress}" -i "{video_file}" {watermark[0]}" -c:v {codec[0]} -metadata 'title=Encoded By [@Anime_Sensei_Network]' -metadata:s:v title="[@Anime_Sensei_Network]" -metadata:s:a title="[@Anime_Sensei_Network]" -metadata:s:s title="[@Anime_Sensei_Network]" -pix_fmt yuv420p -crf {crf[0]} -s {resolution[0]} -c:a libopus -b:a {audio_b[0]} -preset {preset[0]} -c:s copy -map 0 -ac 2 -ab 32k -vbr 2 -level 3.1 -threads 1 "{out_put_file_name}" -y'
    #Done !!
